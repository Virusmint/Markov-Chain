...
Furthermore, the three aforementioned properties of communication allow us to divide the state space of any Markov process into smaller state communicating networks called \textit{communication classes}. A communication class is a set of states that any state $s_i$ communicates with. Since we know that communication is reflexive, this means that the communication class of any state $s_i$ must at least contain the state $s_i$ in its set. Additionally, according to the symmetric property of communication, we can also assert that two communicating states $s_i$ and $s_j$ belong to the same communication class. Finally, by transitivity, every state in a communication class must communicate with each other. Therefore, the number of communication classes in a Markov chain can range anywhere from one to as many states there are in the finite state space.
...
If the process starts at state $s_1$, then it is easy to see that it can only return to that state after multiples of 2 steps. The same can be said about $s_3$, whereas $s_2$ strictly has a period of 2 steps.
...
\\
	\begin{tcolorbox}
		\subsubsection*{Example 2: Gambler's Ruin}
